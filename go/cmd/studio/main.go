...
import (
	...
	"github.com/goniegonie/hvac-studio/go/internal/studio"
...
func findRepoRoot() (string, error) {
	...
	return findRepoRootFromPaths(cwd, executable)
}

func findRepoRootFromPaths(cwd string, executable string) (string, error) {
	starts := []string{}
	if executable != "" {
		exeDir := filepath.Dir(executable)
		for _, candidate := range []string{exeDir, filepath.Dir(exeDir)} {
			if pathExists(filepath.Join(candidate, "release-manifest.json")) && looksLikeRoot(candidate) {
...
func looksLikeRoot(dir string) bool {
	...
	return pathExists(filepath.Join(dir, "go", "go.mod")) ||
