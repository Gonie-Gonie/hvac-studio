...
function renderInspector() {
  ...
  if (isWorkspaceProject()) {
    container.append(componentEditor(component, componentInspectorActions()));
    container.append(replacementPreviewBlock(component, replacementPreviewContext(), replacementPreviewActions()));
  }
...
function componentContractEditorOptions() {
  return {
    ...
    onSaveParameterDefinition: saveParameterDefinition,
    onDeleteParameterDefinition: deleteParameterDefinition,
    onAddState: addStateDefinition,
    onSaveStateDefinition: saveStateDefinition,
    onDeleteStateDefinition: deleteStateDefinition,
  };
}
...
function componentInspectorActions() {
  return {
    onRename: updateComponentFromInspector,
    onDuplicate: duplicateComponent,
    onOpenCode: openComponentCode,
  };
}

function replacementPreviewContext() {
  return {
    selectedTemplate: selectedComponentTemplate(),
    system: currentSystem(),
    connections: state.detail?.graph?.connections || [],
    mapParameters: state.replacementMapParameters !== false,
  };
}

function replacementPreviewActions() {
  return {
    onMapParametersChange(value) {
      state.replacementMapParameters = value;
      renderInspector();
    },
    onReplace: replaceSelectedComponent,
  };
}

function replacementPreviewFor(component, template) {
  return replacementPreviewForComponent(component, template, replacementPreviewContext());
}
...
function updateWorkspaceHelp(mode) {
  ...
  if (!link) return;
  const href = WORKSPACE_HELP[mode] || "/docs/user/quick-start.md";
  link.href = href;
