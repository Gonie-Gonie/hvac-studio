...
import (
	...
	"github.com/goniegonie/hvac-studio/go/internal/model"
...
func TestDocsRouteServesUserGuide(t *testing.T) {
	server := newTestServer(t)
	body := getRouteBody(t, server, "/docs/user/quick-start.md")
	if !bytes.Contains(body, []byte("# Quick Start")) {
		t.Fatalf("docs route did not serve the quick start guide")
	}

	modelingBody := getRouteBody(t, server, "/docs/user/modeling.md")
	if !bytes.Contains(modelingBody, []byte("# Modeling")) {
		t.Fatalf("docs route did not serve the modeling guide")
	}
}
...
func TestStaticModuleEntrypointServes(t *testing.T) {
	...
	if !bytes.Contains(connectionInspectorBody, []byte("connectionUnitConversionEditor")) ||
		...
		!bytes.Contains(connectionsBody, []byte("UNIT_ALIASES")) ||
		!bytes.Contains(connectionsBody, []byte("btuh_to_kw")) ||
		!bytes.Contains(configBody, []byte("degf_to_degc")) ||
		!bytes.Contains(configBody, []byte("rt_to_kw")) {
