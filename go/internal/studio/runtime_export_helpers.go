...
import (
	...
	"github.com/goniegonie/hvac-studio/go/internal/apperror"
	"github.com/goniegonie/hvac-studio/go/internal/compiler"
	"github.com/goniegonie/hvac-studio/go/internal/platform"
	"github.com/goniegonie/hvac-studio/go/internal/project"
)
...
func writeRuntimeExportSupportFiles(projectRoot string, exportRoot string, options exportOptions) ([]string, error) {
	support := findRuntimeSupport(projectRoot)
	if support.root == "" {
		return []string{}, nil
	}
	files := []string{}
	seen := map[string]bool{}
	for _, name := range []string{"bcs-runner", "bcs-env"} {
		rel := filepath.Join("bin", platform.ExecutableName(name))
		if err := copyExternalExportFileAs(support.root, exportRoot, filepath.Join(support.bin, platform.ExecutableName(name)), rel, &files, seen); err != nil {
			return nil, err
		}
	}
	if err := copyExternalExportFileAs(support.root, exportRoot, support.manifest, "runtime/manifest.json", &files, seen); err != nil {
		return nil, err
	}
	for _, rel := range []string{"schema/serve-request.schema.json", "schema/serve-response.schema.json"} {
		if err := copyExternalExportFile(support.root, exportRoot, rel, &files, seen); err != nil {
			return nil, err
		}
	}
	if err := copyExternalExportDirAs(support.root, exportRoot, support.python, "runtime/python", &files, seen); err != nil {
		return nil, err
	}
	if err := copyExternalExportDir(support.root, exportRoot, "python/bcs_worker", &files, seen); err != nil {
		return nil, err
	}
	if options.IncludeSDKExamples {
		if err := copyExternalExportDir(support.root, exportRoot, "python/bcs_sdk", &files, seen); err != nil {
			return nil, err
		}
	}
	sort.Strings(files)
...
func findRuntimeSupport(start string) runtimeSupport {
	...
	for {
		...
		if _, runnerErr := os.Stat(runner); runnerErr == nil {
			if _, pythonErr := os.Stat(python); pythonErr == nil {
				return runtimeSupport{root: absStart, bin: "bin", manifest: "runtime/manifest.json", python: "runtime/python"}
			}
		}
		if _, moduleErr := os.Stat(filepath.Join(absStart, "go", "go.mod")); moduleErr == nil {
			if _, runnerErr := os.Stat(filepath.Join(absStart, ".tmp", "bin", platform.ExecutableName("bcs-runner"))); runnerErr == nil {
				pythonRoot := filepath.Join(".toolchain", "python")
				versions, _ := os.ReadDir(filepath.Join(absStart, pythonRoot))
				for index := len(versions) - 1; index >= 0; index-- {
					candidate := filepath.Join(pythonRoot, versions[index].Name())
					if _, pythonErr := os.Stat(filepath.Join(absStart, candidate, platform.ExecutableName("python"))); pythonErr == nil {
						return runtimeSupport{root: absStart, bin: ".tmp/bin", manifest: "scripts/release/runtime-manifest.json", python: candidate}
					}
				}
			}
		}
...
func copyExternalExportDir(sourceRoot string, targetRoot string, rel string, files *[]string, seen map[string]bool) error {
	return copyExternalExportDirAs(sourceRoot, targetRoot, rel, rel, files, seen)
}

func copyExternalExportDirAs(sourceRoot string, targetRoot string, sourceRel string, targetRel string, files *[]string, seen map[string]bool) error {
	sourcePath := filepath.Join(sourceRoot, filepath.FromSlash(sourceRel))
	...
	if !info.IsDir() {
		return apperror.Errorf(apperror.CodeValidation, "export support source is not a directory: %s", sourceRel)
	}
	return filepath.WalkDir(sourcePath, func(path string, entry fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		entryRel, err := filepath.Rel(sourcePath, path)
		if err != nil {
			return err
		}
		if entryRel == "." {
		...
		return copyExternalExportFileAs(sourceRoot, targetRoot, filepath.Join(sourceRel, entryRel), filepath.Join(targetRel, entryRel), files, seen)
	})
}
...
func copyExternalExportFile(sourceRoot string, targetRoot string, rel string, files *[]string, seen map[string]bool) error {
	return copyExternalExportFileAs(sourceRoot, targetRoot, rel, rel, files, seen)
}

func copyExternalExportFileAs(sourceRoot string, targetRoot string, sourceRel string, targetRel string, files *[]string, seen map[string]bool) error {
	if sourceRel == "" || sourceRel == "." {
		return nil
	}
	artifactPath := filepath.ToSlash(targetRel)
	if seen[artifactPath] {
		return nil
	}
	sourcePath := filepath.Join(sourceRoot, filepath.FromSlash(sourceRel))
