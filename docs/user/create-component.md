...
New components use the generated-wrapper layout by default. Studio and the runner own the wrapper contract, while the Code workspace opens `user_step.py` as the primary editable function body. Source save APIs write only that editable user step for generated-wrapper components, leaving `wrapper.py` and component metadata as Studio-managed runtime artifacts. When node, parameter, or state contracts change in Studio, `component.json` and the class-level contract metadata in `wrapper.py` are regenerated together while the user step body is preserved. Existing single-file components still load and can be edited for compatibility with older projects.

Available component templates include scalar, controller, stateful, data source, data sink, utility, feature mapper, ML inference, zone-load ANN, vectorized, external executable, and solver boundary. The Project panel filters category and execution-mode choices to the installed templates, so unavailable template types are not shown as empty choices. Vectorized components use `execution_mode: "vectorized"` and may implement `evaluate_batch(inputs, state, params, context)` with the same `(outputs, state)` return contract as `evaluate`. ML inference components use `ml_metadata` to record model assets, feature schemas, target schemas, validation reports, valid ranges, and package requirements. External executable components use `kind: "external_exe"` with `execution_mode: "external_executable"` and run the process named by `parameters.command` and `parameters.args`. Solver boundary components use `category: "solver"` and `solver_boundary` metadata to keep iterative feedback behavior inside one component.
...
Workspace component input and output nodes can be added, edited, renamed, direction-changed, or deleted from the Inspector. New nodes can start from presets for water inlet/outlet, air inlet/outlet, control signal input, electric power output, scalar input/output, and time-series input, then be adjusted with IDs, display names, directions, media, value types, units, required flags, and input defaults. If the component is already in the runnable system, new input nodes are exposed as public inputs and added to the default input file; new output nodes are exposed as public outputs. Editing node metadata updates related public IO, and editing an input default updates the default input file. Renaming a node updates public IO targets and connection endpoints after a confirmation that summarizes the affected references and loaded source-body matches. Changing a node between input and output removes now-invalid connections, moves the node between public input/output surfaces, updates the default input file, and restores downstream public inputs when an output connection is removed. Deleting a node removes related public IO and connection references.
