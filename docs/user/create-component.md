...
Workspace component input and output nodes can be added, edited, renamed, or deleted from the Inspector. New nodes can start from presets for water inlet/outlet, air inlet/outlet, control signal input, electric power output, scalar input/output, and time-series input, then be adjusted with IDs, display names, media, value types, units, required flags, and input defaults. If the component is already in the runnable system, new input nodes are exposed as public inputs and added to the default input file; new output nodes are exposed as public outputs. Editing node metadata updates related public IO, and editing an input default updates the default input file. Renaming a node updates public IO targets and connection endpoints after a confirmation that summarizes the affected references and loaded source-body matches. Deleting a node removes related public IO and connection references.
