...
| PM-106 | Run/inspect | In progress: component timing bars, component stdout/stderr logs, and failed-run/batch problem summaries are in place; run comparison, trace timelines, value charts, cancellation, timeout, and recovery controls still need depth | Repeated run/debug sessions are inspectable without reading JSON |
