...
| PM-106 | Run/inspect | In progress: component timing bars, run-to-run public output comparison, component stdout/stderr logs, configurable run/batch timeout/cancel controls, and failed-run/batch problem summaries are in place; trace timelines, richer value charts, and deeper recovery controls still need depth | Repeated run/debug sessions are inspectable without reading JSON |
