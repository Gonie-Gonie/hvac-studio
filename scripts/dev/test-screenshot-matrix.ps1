...
if (-not $OutputRoot) {
  $OutputRoot = Join-Path $RepoRoot '.tmp\screenshot-matrix\latest'
}
...
function Assert-ScreenshotStaticCoverage {
  ...
  @(
    'go\internal\studio\static\index.html',
    'go\internal\studio\static\styles.css'
  ) | ForEach-Object { $StaticFiles.Add((Join-Path $RepoRoot $_)) }
  Get-ChildItem -LiteralPath (Join-Path $RepoRoot 'go\internal\studio\static\js') -Filter '*.js' -File |
