...
func TestRunAlternativeOptimizationAlgorithmsUseRunnerCandidateFlow(t *testing.T) {
	tmpDir := t.TempDir()
	projectRoot := filepath.Join(tmpDir, "project")
	copyTree(t, filepath.Join("..", "..", "..", "..", "examples", "006_optimization_case"), projectRoot)
	projectPath := filepath.Join(projectRoot, "project.bcsproj")
	...
	for _, algorithm := range []string{"differential_evolution", "custom_sdk_script"} {
		setup.Algorithm = algorithm
		result, err := Run(context.Background(), projectPath, setup, Options{})
		if err != nil {
			t.Fatal(err)
		}
		if !result.OK || result.Algorithm != algorithm || result.BestInputs["chw_setpoint_c"] != 7.0 {
			t.Fatalf("%s optimization result = %#v", algorithm, result)
		}
	}
}
...
func TestRunGridOptimizationAppliesSystemParameterVariables(t *testing.T) {
	tmpDir := t.TempDir()
	projectRoot := filepath.Join(tmpDir, "project")
	copyTree(t, filepath.Join("..", "..", "..", "..", "examples", "006_optimization_case"), projectRoot)
	projectPath := filepath.Join(projectRoot, "project.bcsproj")
	...
	setup := Setup{
		ID:        "system_parameter_credit_grid",
		Name:      "System Parameter Credit Grid",
		Algorithm: "grid",
		BaseInputs: map[string]any{
			"building_load_kw": 500.0,
			"chw_setpoint_c":   7.0,
	...
	result, err := Run(context.Background(), projectPath, setup, Options{
		SaveParameterSet: filepath.Join("parameter_sets", "optimized_system_credit.json"),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if result.SavedParameterSet != "parameter_sets/optimized_system_credit.json" {
		t.Fatalf("saved parameter set = %q", result.SavedParameterSet)
	}
}
