...
function renderStartWorkflowRows() {
  const tbody = el("startWorkflowRows");
  if (!tbody) return;
  tbody.innerHTML = "";
  const rows = workflowReadinessRows();
  if (!rows.length) {
    tbody.append(emptyRow(4, "No project open"));
    return;
  }
...
function workflowReadinessRows() {
  ...
  return [
    readinessRow(
      ...
      "Ready",
      "Blocked",
      isWorkspaceProject() ? currentProject()?.relative_path || state.detail.project_path || "Workspace project" : "Open or copy a workspace project",
      "start",
    ),
    readinessRow(
      "System Graph",
...
function readinessRow(name, ok, okStatus, blockedStatus, detail, mode, action = "Open", disabled = false, levelOverride = "") {
